...
public class Barrier5 extends Barrier{
	
	public Barrier5(World world,Box2D box2d, WYPoint point){
		
		super(world, box2d);
		
		bodyBarrier = mLoader.createBodyByName(mBox2d, "barrier5");
		bodyBarrier.setType(Body.TYPE_KINEMATIC);
		
			
		bodyBarrier.setTransform(point.x, point.y, 0);

		Sprite sprite = Sprite.make(R.drawable.barrier5);
		sprite.autoRelease();
		
		WYPoint anchor = mLoader.getAnchorPercent("barrier5");
