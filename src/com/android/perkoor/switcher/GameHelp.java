...
public class GameHelp extends Scene {
	...
	Button backButton;
	public GameHelp(Scene gameHomepage) {
		this.gameHomepage = gameHomepage;
		WYSize size = Director.getInstance().getWindowSize();
		gamehelp = Sprite.make(R.drawable.help_background);
		float scaleX = size.width / gamehelp.getWidth();
		float scaleY = size.height / gamehelp.getHeight();
		gamehelp.setScale(scaleX, scaleY);
		gamehelp.setPosition(size.width / 2, size.height / 2);
	...
	public boolean onBackButtonClicked() {
		Director.getInstance().replaceScene(ZoomFlipAngularTransition.make(1f, gameHomepage, false, 0.5f));
		return true;
